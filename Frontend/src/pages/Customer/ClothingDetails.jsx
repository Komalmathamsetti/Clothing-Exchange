...
export default function ClothingDetails() {
  ...
  const openSwapModal = async () => {
    try {
      ...
      toast.error(
        error.response?.data?.message || "Failed to load your clothes",
      );
    }
  };
  ...
  useEffect(() => {
    const fetchClothing = async () => {
      try {
        ...
        console.log(error);
        toast.error(
          error.response?.data?.message || "Unable to get the clothing",
        );
      } finally {
        setLoading(false);
      }
    ...
    <DashboardLayout user={user} showNavbar={true}>
      <main className="min-h-screen bg-slate-100 px-4 py-6 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <button
            onClick={() => navigate("/browse-clothes")}
            className="mb-6 inline-flex items-center gap-2 rounded-xl px-3 py-2 text-sm font-medium text-slate-600 hover:bg-white hover:text-emerald-700"
          >
            <ArrowLeft size={18} />
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <div className="rounded-3xl bg-white p-6 shadow-sm lg:min-h-155">
              {clothing.images && clothing.images.length > 0 ? (
                ...
                <div className="flex min-h-125 w-full items-center justify-center rounded-2xl bg-slate-100 text-slate-400">
                  <div className="text-center">
                    <ImageIcon className="mx-auto mb-3 h-16 w-16" />
                    <p className="text-sm">No clothing image available</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="rounded-3xl bg-white p-6 shadow-sm sm:p-8">
              ...
              <div className="mt-6 flex items-center gap-2 text-slate-600">
                ...
                <span>
                  {clothing.city || "Location Unavailable"},{" "}
                  {clothing.state && `,${clothing.state}`}
                </span>
              </div>
              ...
              <button
                type="button"
                disabled={clothing.status?.toUpperCase() !== "AVAILABLE"}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-lg rounded-3xl bg-white p-6 shadow-2xl sm:p-8">
            ...
            <div className="mt-6 rounded-2xl bg-emerald-50 p-4">
              ...
              <p className="mt-1 font-bold text-slate-900">{clothing.title}</p>
