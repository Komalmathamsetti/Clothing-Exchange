  ...
  const fetchProfile = async () => {
    try {
      ...
      setDashboard({
        ...
        recentListings: dashboardResponse.data.recentListings || [],
      });
    } catch (error) {
      ...
      <div className="space-y-8">
        <section className="relative overflow-hidden rounded-3xl bg-emerald-600 p-7 text-white shadow-xl shadow-emerald-100 sm:p-10">
          <div className="relative z-10 max-w-2xl">
            ...
            <p className="mt-3 text-sm leading-6 text-emerald-50 sm:text-base">
              Manage your clothing swaps and help build a sustainable fashion
              community.
            </p>
            <Link
              to="/add-clothing"
              className="mt-7 inline-flex rounded-xl bg-white px-5 py-3 text-sm font-bold text-emerald-700 hover:bg-emerald-50"
            >
              + Add New Clothing
            </Link>
          </div>
        ...
        <section className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-5">
            ...
            <p className="mt-1 text-3xl font-bold">
              {dashboard.stats.completedSwaps}
            </p>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-white p-5">
            <div className="flex justify-between text-2xl">
        ...
        <section className="grid gap-6 xl:grid-cols-5">
          <div className="rounded-2xl border border-slate-200 bg-white p-6 xl:col-span-3">
            ...
            {dashboard.recentActivity.length === 0 ? (
              ...
              dashboard.recentActivity.map((activity) => {
                const isReceiver =
                  Number(activity.reciever_id) === Number(user?.id);
                ...
                const itemName = isReceiver
                  ? activity.reciever_item_title
                  : activity.sender_item_title;
                return (
                  <div
                    key={activity.id}
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 xl:col-span-2">
            ...
            <p className="mt-1 text-sm text-slate-400">
              Jump back into your wardrobe
            </p>

            <div className="mt-6 grid grid-cols-2 gap-3">
              <Link
                to="/add-clothing"
                className="rounded-2xl border border-slate-200 p-4 hover:border-emerald-300 hover:bg-emerald-50"
              >
              ...
              <Link
                to="/nearby-swaps"
                className="col-span-2 rounded-2xl border border-slate-200 p-4 hover:border-emerald-300 hover:bg-emerald-50"
              >
                <span className="text-xl">📍</span>
              ...
              >
                ...
                <p className="mt-3 text-sm font-semibold">My Disputes</p>
              </Link>
              <Link
                to="/raise-dispute"
                className="rounded-2xl border border-slate-200 p-4 hover:border-emerald-300 hover:bg-emerald-50"
              >
                <span className="text-xl">📝</span>
                <p className="mt-3 text-sm font-semibold">Raise Dispute</p>
              </Link>
            </div>
          </div>
        </section>
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
          <div className="flex items-center justify-between border-b border-slate-100 p-6">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-400">
                Manage your recently added pieces
              </p>
            </div>
            <Link
              to="/my-listings"
